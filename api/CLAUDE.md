...
- **`CLERK_JWKS_URL` must be the JWKS document, not the instance root.** The Clerk domain answers `200` with an empty body, so a bare `https://<instance>.clerk.accounts.dev` produces no keys and every signed-in request 401s with `"not authenticated"` — the frontend then reports "Your guest reports haven't been moved to this account yet." on every page load. The path is `/.well-known/jwks.json`.
- **`ClerkVerifier` pins certifi's CA bundle.** A CPython build with no CA store (the python.org installer, before its certificate script is run) fails the JWKS fetch with `CERTIFICATE_VERIFY_FAILED`. Because `_verify_sync` maps *any* exception to "not authenticated", that misconfiguration is indistinguishable from a forged token. The same reasoning is why the failure logs at `warning`, not `info`: at `info` it is invisible under the default level.
